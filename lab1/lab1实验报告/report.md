...
-本实验涉及操作系统启动阶段的核心机制。在实验层面，我们通过调试观察了RISC-V硬件从复位向量0x1000到内核入口0x80200000的完整执行路径，包括固件初始化、参数传递和控制权转移过程。这对应操作系统原理中的系统引导概念，体
现了硬件自检、环境初始化和内核加载的标准流程。

-实验中通过链接脚本定义的内存布局，特别是内核代码被固定在特定物理地址的现象，直接关联操作系统原理中的内存管理基础。

-设备树传递硬件配置信息的方式，展示了操作系统原理中硬件抽象层的具体实现。这种描述性配置使得内核能够以统一接口访问多样化硬件，体现了设备驱动模型的核心思想。

-执行模式从机器模式到监督者模式的切换，对应操作系统原理中的特权级保护机制。虽然实验中的切换相对简单，但揭示了CPU通过特权级别实现资源保护和系统安全的根本原理。

### 3-2 **实验仍未涉及的部分**

-进程管理与调度作为操作系统的核心功能，包括进程控制块、上下文切换和调度算法等机制在启动阶段并未体现。

-文件系统相关的存储管理、目录结构和缓存机制也超出了当前实验范围。

-系统调用作为用户程序与内核的交互接口，其陷入机制和参数传递规范在纯内核环境中尚未涉及。

-完整的内存分页与虚拟地址管理，包括页表设置和缺页处理等复杂机制，在当前直接使用物理内存的简单模型中还未实现。
