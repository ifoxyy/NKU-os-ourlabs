...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 IMPLEMENTATION
    {
        uintptr_t cur = offset;
        uintptr_t last = endpos;
        while (cur < last) {
            uint32_t bno = cur / SFS_BLKSIZE;
            off_t boff = cur % SFS_BLKSIZE;
            uintptr_t to = (last < (uintptr_t)((bno + 1) * SFS_BLKSIZE) ? last : (uintptr_t)((bno + 1) * SFS_BLKSIZE));
            size = to - cur;

            if ((ret = sfs_bmap_load_nolock(sfs, sin, bno, &ino)) != 0) {
                goto out;
            }

            if (boff == 0 && size == SFS_BLKSIZE) {
                /* aligned full block */
                if ((ret = sfs_block_op(sfs, (char *)buf + alen, ino, 1)) != 0) {
                    goto out;
                }
            } else {
                /* partial block */
                if ((ret = sfs_buf_op(sfs, (char *)buf + alen, size, ino, boff)) != 0) {
                    goto out;
                }
            }
