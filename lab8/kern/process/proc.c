...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        proc->lab6_stride = 0;
        /* default priority must be >=1 to avoid division by zero in stride */
        proc->lab6_priority = 1;

        /* LAB8: initialize files pointer (file descriptor table) */
        proc->filesp = NULL;
...
set_links(struct proc_struct *proc)
{
    ...
    proc->yptr = NULL;
    struct proc_struct *old_head = proc->parent->cptr;
    if ((proc->optr = old_head) != NULL)
    {
        proc->optr->yptr = proc;
    }
...
remove_links(struct proc_struct *proc)
{
    ...
    nr_process--;
    (void)nr_process;
    /* clear pointers in removed proc to avoid dangling links */
    proc->optr = proc->yptr = NULL;
    proc->parent = NULL;
}
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    if ((proc = alloc_proc()) == NULL) {
        ...
        /* inherit lab6 priority/stride from parent so enqueue has valid priority */
    ...
    // 5. copy_files (LAB8): duplicate or share file descriptor table
    if (copy_files(clone_flags, proc) != 0) {
        goto bad_fork_cleanup_kstack;
    }

    // 6. insert proc_struct into hash_list && proc_list, set the relation links of process
    proc->pid = get_pid();      // allocate unique pid
    hash_proc(proc);
    set_links(proc);

    // 7. make the new child process RUNNABLE
    wakeup_proc(proc);

    // 8. set return value using child proc's pid
    ret = proc->pid;
...
int do_exit(int error_code)
{
    ...
    local_intr_save(intr_flag);
    {
        ...
        {
            proc = current->cptr;
            struct proc_struct *next = proc->optr;

            /* Detach proc from current parent's child list */
            if (proc->optr != NULL)
            {
                proc->optr->yptr = proc->yptr;
            }
            ...
            /* Insert proc at head of initproc's child list */
            proc->yptr = NULL;
            proc->optr = initproc->cptr;
            if (initproc->cptr != NULL)
            {
...
load_icode(int fd, int argc, char **kargv)
{
    ...
    struct proghdr *ph_table = NULL;
    if (elf.e_phnum > 0) {
        size_t ph_size = elf.e_phnum * sizeof(struct proghdr);
        if ((ph_table = kmalloc(ph_size)) == NULL) {
            ret = -E_NO_MEM;
            goto bad_elf_cleanup_pgdir;
        }
        if ((ret = load_icode_read(fd, ph_table, ph_size, elf.e_phoff)) != 0) {
            goto bad_cleanup_ph_table;
        }
    }
    ...
    for (; ph < ph_end; ph++) {
        ...
        while (start < end_file) {
            ...
            la += PGSIZE;
            if (end_file < la) size -= la - end_file;

            // read from file at offset ph->p_offset + bytes_read
            if ((ret = load_icode_read(fd, page2kva(page) + off, size, ph->p_offset + bytes_read)) != 0) {
                goto bad_cleanup_mmap;
            }
            start += size;
            bytes_read += size;
        ...
        // bss part: zero-fill remaining
        uintptr_t end_mem = ph->p_va + ph->p_memsz;
        if (start < la) {
            // last allocated page may need partial zero
            if (start == end_mem) {
                continue;
            }
            if (page == NULL) goto bad_cleanup_mmap;
            size_t off = start + PGSIZE - la;
            size_t size = PGSIZE - off;
            if (end_mem < la) size -= la - end_mem;
...
repeat:
    ...
    if (haskid)
    {
        /* sleeping until child changes state */
...
found:
    ...
    local_intr_restore(intr_flag);
    /* clear wait_state now that we've successfully reclaimed a child */
    current->wait_state = 0;
    put_kstack(proc);
    kfree(proc);
    return 0;
...
void lab6_set_priority(uint32_t priority)
{
    ...
    /* Switch global scheduler to stride scheduler on first use of lab6 API */
    extern void sched_set_class_stride(void);
    sched_set_class_stride();
    /* If this process is already runnable and in run-queue, update its position */
    extern void sched_update_proc(struct proc_struct *proc);
    sched_update_proc(current);
}
