...
- 要观察「TLB 命中/未命中」：在 [cputlb.c](vscode-file://vscode-app/d:/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 的 `tlb_hit(...)` 检查点和 `victim_tlb_hit(...)` 内下断点。
...
- 要观察「SATP / VM 模式 / TL B flush 影响」：查看 [csr.c](vscode-file://vscode-app/d:/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 的 `write_satp`（SATP 改变时有分支触发 tlb flush 逻辑）。
